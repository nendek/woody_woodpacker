...
void		test_packer(t_info *info, void *original)
{
	void	*test;

	test = mmap(NULL, info->file_size, PROT_WRITE | PROT_READ, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	ft_memcpy(test, original, info->file_size);
	mprotect(test, info->file_size, PROT_EXEC);
	void (*func)();
	func = (void (*)()) test + info->base_entry;
	(void)func;
	(void)(*func)();
}
