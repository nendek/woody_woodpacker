...
static size_t       get_padding_size(t_info *info, Elf32_Phdr *program_header) 
{
	Elf32_Phdr  *crawler;
	int32_t     i;  
	size_t      next_offset;
	...
	next_offset = -1; 
	if (!(crawler = (Elf32_Phdr *)(move_ptr(info, info->file, sizeof(Elf32_Ehdr)))))
		return (1);
	i = 0;
	found = 0;
	while (i < info->nb_hp)
	{   
		...
		i++;
		if (!(crawler = move_ptr(info, crawler, sizeof(Elf32_Phdr))))
			return (1);
	}   
	if (found == 0)
		return (0);
...
static int32_t		save_place_bss(t_info *info)
{
	...
	if (!(header = get_last_load32(info, info->file)))
		return (1);
...
static int32_t		save_place_padding(t_info *info, Elf32_Phdr *program_header, int32_t nb_segment)
{
...
static int32_t		save_place_3(t_info *info, Elf32_Phdr *program_header)
{

	if (get_padding_size(info, program_header) < info->woody_size + (info->push_size * 2)) // TODO : get define 1 = push all size
		return (1);
...
static int32_t		save_place_text(t_info *info, Elf32_Phdr *program_header)
{
...
int32_t		get_case32_1(t_info *info)
{
	...
	i = 0;
	if (!(program_header = (Elf32_Phdr *)(move_ptr(info, info->file, sizeof(Elf32_Ehdr)))))
		return (1);
	while (i < info->nb_hp)
	{
		if (program_header->p_type == PT_LOAD)
